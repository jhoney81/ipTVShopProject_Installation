...
public class InstallationController {
    ...
    @RequestMapping(method=RequestMethod.PATCH, path="/installations/visit")
    public void visitationCompletion(@RequestParam (value="orderId", required=false, defaultValue="0") Long orderId) {

        Installation visitationCompl = installationRepository.findByOrderId(orderId);
        visitationCompl.setStatus("VISITATIONCOMPLETED");
        SimpleDateFormat defaultSimpleDateFormat = new SimpleDateFormat("YYYYMMddHHmmss");
        String today = defaultSimpleDateFormat.format(new Date());
        visitationCompl.setVisitCompleteDate(today);
        installationRepository.save(visitationCompl);
