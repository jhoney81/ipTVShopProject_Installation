...
public class PolicyHandler{
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void wheneverAsAccepted_VisitationRequest(@Payload AsAccepted asAccepted){

        if(asAccepted.isMe()){

            Installation visitationAccept = new Installation();
            visitationAccept.setStatus("VISITATIONACCEPTED");
            SimpleDateFormat defaultSimpleDateFormat = new SimpleDateFormat("YYYYMMddHHmmss");
            String today = defaultSimpleDateFormat.format(new Date());
            visitationAccept.setVisitationDate(today);
            visitationAccept.setEngineerId(asAccepted.getEngineerId());
            visitationAccept.setEngineerName(asAccepted.getEngineerName());
            visitationAccept.setOrderId(asAccepted.getOrderId());

            installationRepository.save(visitationAccept);
            System.out.println("##### listener VisitationRequest : " + asAccepted.toJson());
        }
    }
